...
router.post("/following/boards", (req, res) => {
  ...
  if (req.isAuthenticated()) {
    User.findByIdAndUpdate(
      req.user._id,
      { $push: { followed_boards: board_to_follow } },
      (err, result) => {
        res.json({
          err: err,
          result: result
...
router.delete("/following/boards/:id", (req, res) => {
  ...
  if (req.isAuthenticated()) {
    User.findByIdAndUpdate(
      req.user._id,
      { $pullAll: { followed_boards: [board_to_delete] } },
      (err, result) => {
        res.json({
          err: err,
          result: result
...
router.delete("/following/user/:id", (req, res) => {
  ...
  if (req.isAuthenticated()) {
    User.findByIdAndUpdate(
      req.user._id,
      { $pullAll: { following: [user_to_delete] } },
      (err, result) => {
        res.json({
          err: err,
          result: result
...
router.post("/search/boards", (req, res) => {
  if (req.isAuthenticated()) {
    ...
    let results = [];
    Board.find(
      { creator: String(req.user._id), $text: { $search: searchTerms } },
...
router.post("/search/pins", (req, res) => {
  if (req.isAuthenticated()) {
    let searchTerms = req.body.search_string;
    Pin.find({ $text: { $search: searchTerms }, creator: req.user._id }).exec(
      (err, results) => {
        res.send(results);
      }
    );
